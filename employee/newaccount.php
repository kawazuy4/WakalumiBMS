...
if (isset($_POST['insert'])) {
    ...
    $sqli_qery = "SELECT nik, email from accountsholder where nik='$nik' or email='$email'";
    ...
    if ($dup_nik == $nik) {
        ...
        $sql = "INSERT INTO accountsholder(account, name, birth_place, nik, contact, dob, gender, email, postal, city, house_address, npwp) VALUES ('$acc_full','$name','$birth_place','$nik','$contact','$dob','$gender','$email','$postal','$city','$address','$npwp')";
        $current_id = mysqli_query($con, $sql) or die(mysqli_error($con));
        if (isset($current_id)) {
            mysqli_query($con, "UPDATE counting set acc_count='$acc_num'");
            $sqli = "INSERT INTO accounts_info(account,account_title,account_type,balance,register_date) VALUES ('$acc_full','$title','$type','$balance','$regisdate')" or die(mysqli_error($con));
            ...
            if ($connected) {
                $msg = "Halo Pengguna " . $name . "! Nasabah yang terdaftar memiliki saldo sebesar Rp " . $tms1 . ". " . $balance . ". Deposit dengan sukses!";
            }
            $_SESSION["title"] = "Done";
        ...
        <section class="content" id="top">
            <div class="container-fluid">
                <div class="block-header">
                    <div class="col-sm-8">
                        <p style="margin-left: -15px; font-size: 17px; font-weight: bold;">FORMULIR PENDAFTARAN REKENING BARU</p>
                    </div>
                    <div class="col-sm-4">
                ...
                <form id="form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        ...
                        <hr
                            style="height:1px;border-width:0; width: 100%; margin-bottom:-5px; color:red;background-color:gray;">
                        <br>
                        <p style="text-align: left; font-weight: bold;">Detail Rekening</p>
                        <div class="row">
                            <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Nama Akun</p>
                                <input type="text" class="form-control" name="title" placeholder="Input Nama Akun"
                                    required>
                            </div>
                            <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Saldo Rekening</p>
                                <input type="number" class="form-control" id="blnc" name="balance" min="100000"
                                    placeholder="Saldo Awal Minimal Rp. 100.000,00-" required>
                            </div>
                            <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Tipe Rekening
                                </p>
                                <select class="form-control" name="type" required>
                                    <option selected disabled hidden value="">Pilih Tipe Rekening</option>
                                    <option value="Tabungan">Rekening Tabungan</option>
                                    <option value="Giro">Rekening Giro</option>
                                </select>
                            </div>
                        </div>
                        <hr
                            ...
                            <div class="col-sm-12">
                                <center>
                                    <button type="submit"
                                        ...
                                        class="btn btn-primary" name="insert">Daftarkan Rekening</button>
                                </center>
                            </div>
                        </div>
                    </div>
