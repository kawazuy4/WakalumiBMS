  ...
  <section class="content" id="top">
    <div class="container-fluid">
      <div class="block-header">
        <div class="col-sm-8">
          <p style="margin-left: -15px; font-size: 17px; font-weight: bold;">RIWAYAT PINJAMAN</p>
        </div>
      </div>
      <hr
      ...
      if ($_GET['id'] == "") {
        ?>
        <p style="text-align: left; font-weight: bold;">Cari Riwayat Pinjaman</p>
        <form id="form" method="post" enctype="multipart/form-data">
          <div class="row">
            <div class="col-sm-8">
              <input type="text" class="form-control" name="acc" placeholder="Cari Riwayat Berdasarkan Nomor Akun Pinjaman"
                id="acc" required>
            </div>
            <div class="col-sm-2">
      ...
      if (isset($_POST['filter']) || isset($_POST['dt']) || $dd != "") {
        ...
        $sql1 = "SELECT * FROM loanaccount_history  WHERE loan_account='$acc'";
        ...
        if (mysqli_num_rows($nm1) > 0) {
          ...
          <div class="row">
            <div class="col-sm-4">
              <select class="form-control" onchange="myfun()" id="tp">
                ...
                <option value="BAYAR ANGSURAN">Bayar Angsuran</option>
            ...
            <form method="post" enctype="multipart/form-data">
              <div class="col-sm-3">
                ...
                <input type="hidden" name="sd" value="<?php echo $row1['loan_account']; ?>">
              </div>
              <div class="col-sm-3">
          ...
          <div class="box-body">
            <?php
            if (isset($_POST['filter'])) {
              $sql = "SELECT c.* , p.* FROM loan_accountsholder c, loanaccount_history p WHERE c.loan_account = p.loan_account and p.loan_account='$acc' ORDER BY p.no DESC";
            } else if (isset($_POST['dt'])) {
              $sql = "SELECT c.* , p.* FROM loan_accountsholder c, loanaccount_history p WHERE c.loan_account = p.loan_account and p.loan_account='$acc' and p.dt between '$st' and '$et' ORDER BY p.no DESC";
            } else if (isset($dd)) {
              $sql = "SELECT c.* , p.* FROM loan_accountsholder c, loanaccount_history p WHERE c.loan_account = p.loan_account and p.loan_account='$acc' ORDER BY p.no DESC";
            }
              ...
              <table class="table" id="myTable">
                <thead>
                  <tr style="text-align: center;" class="tb">
                    ...
                    <th style="text-align: center; width: 12%;">Nomor Akun Pinjaman</th>
                  ...
                  if (mysqli_num_rows($nm) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                      ...
                      <tr>
                        ...
                        <td style="text-align: center;padding-top: 25px;"><?php echo $row['loan_account']; ?></td>
                    ...
                    ?>
                    <tr>
                      <td style="text-align: center;padding-top: 25px; font-size: 20px;" colspan="7">Riwayat tidak ditemukan
                      </td>
                    </tr>
                    <?php
                  } ?>
                  <tr class='no-records' style="display: none; text-align: center;padding-top: 25px; font-size: 20px;">
                    <td colspan='7'>Riwayat tidak ditemukan</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    function myfun() {
      ...
      for (i = 0; i < tr.length; i++) {
        ...
        if (td) {
          ...
          if (filter == "PINJAM" || filter == "BAYAR ANGSURAN") {
            ...
            if (txtValue.toUpperCase().indexOf("PINJAM") > -1 || txtValue.toUpperCase().indexOf("BAYAR ANGSURAN") > -1) {
              tr[i].style.display = "";
            } else {
              tr[i].style.display = "none";
            }
